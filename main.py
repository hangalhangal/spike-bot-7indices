...
def allowed(k,c,conf):
    if models[k]["samples"]<MIN_TRAINING_SAMPLES:
        diag[k]["blocked_training"]+=1;return False
    if c==0:return False
    if conf<SIGNAL_CONFIDENCE:
        diag[k]["blocked_confidence"]+=1;return False
    if time.time()-last_signal[k]<COOLDOWN_SECONDS:
        diag[k]["blocked_cooldown"]+=1;return False
    return True
...
async def send_signal(app,k,direction,conf,probs):
    ...
    for cid in list(chat_ids):
        ...
        except Exception as e:logging.error("Telegram send: %s",e)
...
async def start(update,context):
    ...
    for k in INDICES:
        active.add(k)
        if k not in tasks or tasks[k].done():tasks[k]=asyncio.create_task(start_ws(INDICES[k]["symbol"],k,context.application))
...
async def status(update,context):
    chat_ids.add(update.effective_chat.id)
    msg="🧠🔥 AI MANIAC V5 FIX STATUS\n━━━━━━━━━━━━━━━━━━\n\n"
    for k,info in INDICES.items():
        ok=stats[k]["ok"];fail=stats[k]["fail"];total=ok+fail;wr=ok/total*100 if total else 0;p=last_probs[k]
        msg+=(f"{'🟢' if k in active else '⚪'} {info['name']}\n"
              f"Ticks: {len(ticks_data[k])}/{LIVE_HISTORY} | LIVE TICKS: {diag[k]['ticks']}\n"
              f"Features: {diag[k]['features']}\nAI Training: {models[k]['samples']}\n"
              f"WIN: {ok} | LOSS: {fail} | WinRate: {wr:.1f}%\n"
              f"Pending: {len(pending[k])} | Candidates: {diag[k]['candidates']}\n"
