...
async def get_active_symbols(ws):
    """Get current Deriv underlying symbols using the current API fields."""
    await ws.send(json.dumps({"active_symbols":"brief","req_id":9001}))
    while True:
        raw=await ws.recv()
        msg=json.loads(raw)
        if msg.get("msg_type")=="error":
            raise RuntimeError(msg.get("error",{}).get("message","active_symbols error"))
        ...
        for item in msg.get("active_symbols",[]):
            ...
            name_norm=" ".join(str(name).upper().split())
            for key,info in INDICES.items():
                if name_norm==info["name"].upper():
                    found[key]=symbol
        missing=[k for k in INDICES if k not in found]
        if missing:
            raise RuntimeError("Active symbol not found: "+", ".join(missing))
