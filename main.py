        ...
        async with websockets.connect(uri,ping_interval=20,ping_timeout=20,close_timeout=10) as ws:
            ...
            while k in active:
                ...
                if msg_type=="tick" and msg.get("tick"):
                    ...
                    tick_data = msg["tick"]
                elif msg_type=="history" and msg.get("history"):
                    # ticks_history subscribe returns history with single tick in some cases
                    h = msg.get("history",{})
                    prices = h.get("prices",[])
                    times = h.get("times",[])
                    if prices and times:
...
async def symbols(update,context):
    ...
    for k,info in INDICES.items():
        lines.append(f"{info['name']}: {DERIV_SYMBOLS.get(k) or KNOWN_DERIV_SYMBOLS.get(k) or 'NOT RESOLVED'}")
