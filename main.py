...
async def deriv_ws(k,app):
    ...
    try:
        ...
        # === CONNECTION 2: LIVE - V5.2 FIX ===
        async with websockets.connect(uri,ping_interval=20,ping_timeout=20,close_timeout=10) as ws:
            diag[k]["connected"]=1
            diag[k]["stage"]="SUBSCRIBING"
            symbol = KNOWN_DERIV_SYMBOLS[k]
            # ЭНЭ НЬ ГОЛ ЗАСВАР - ticks биш ticks_history
            await ws.send(json.dumps({
                "ticks_history":symbol,
                "adjust_start_time":1,
                "count":1,
            ...
            while k in active:
                ...
                if msg.get("msg_type")=="tick" and msg.get("tick"):
                    tick_data = msg["tick"]
                elif msg.get("msg_type")=="history" and msg.get("tick"):
                    tick_data = msg["tick"]
                elif msg.get("msg_type")=="history" and msg.get("history"):
                    h = msg.get("history",{})
                    prices = h.get("prices",[]); times = h.get("times",[])
                    if prices and times:
                        tick_data = {"quote": prices[-1], "epoch": times[-1]}
                if tick_data:
        ...
        diag[k]["errors"]+=1; diag[k]["last_error"]=f"{type(e).__name__}: {e}"; diag[k]["stage"]="ERROR"
        raise
    finally:
...
def keep_alive():
    ...
    class H(BaseHTTPRequestHandler):
        def do_GET(self):self.send_response(200);self.end_headers();self.wfile.write(b"AI MANIAC V5.2 FIXED LIVE")
...
async def start(update,context):
    ...
    await update.message.reply_text("🧠🔥 AI MANIAC V5.2 FIXED\n\n✅ 7 индекс идэвхжлээ.\n📡 Live tick: ticks_history-оор засварлагдсан.\n💾 Memory ON.\n🎯 40% threshold.",reply_markup=buttons())
